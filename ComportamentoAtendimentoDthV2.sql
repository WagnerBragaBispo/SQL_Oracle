...
para o indicador só acessa e liga e acessa, precisaremos abrir o acesso por canal, para que saibamos qual o canal que o cliente que só acessa usa e qual o canal que o cliente que liga e acessa busca. 
	...
	MUDANÇAS:
		- QUERY VERSÃO 0 PARA V1 --->>> FOI INCLUSO NOS ACESSOS DO IDM O "CLIENT_ID = MINHA_NET_WCP" COMO ACESSO DO SITE
		- TABELA TMP_SQDAA_COMP_ATM_DTH_P4A; ANTES OPS_ALTERYX.BI_FP_CONTRATO_CTV@BASECLARO DEPOIS OPS_ALTERYX.BI_FP_CONTRATO_PRODUTO_CTV@BASECLARO
--====================================================================================================================================================================================
*/
...
SELECT   /*+parallel (32)*/ ----------------------------------------------------------------------------------------------ETAPA 3A BASE DE ACESSOS IDM
  ...
  TO_DATE(
    ...
    ,SUM(CASE WHEN UPPER(IDM.CLIENT_ID) IN ('CLAROTV_WCP_PROD','MINHA_NET_WCP')           THEN 1 ELSE 0 END) AS QT_ACSS_SITE
    ,SUM(CASE WHEN UPPER(IDM.CLIENT_ID) IN ('NETAPP','CLAROTV_WCP_PROD','MINHA_NET_WCP')  THEN 1 ELSE 0 END) AS QT_ACSS_SITE_APP
...
/*----------------------------------------------------------------------------
					resumo 
----------------------------------------------------------------------------*/
SELECT DISTINCT DAT_MOVIMENTO ,CANAL_ATD ,TP_AUTENTICACAO ,COUNT(DISTINCT(NUM_CONTRATO)) QT_CTR_UNICO  FROM TMP_SQDAA_COMP_ATM_DTH_P3B GROUP BY DAT_MOVIMENTO ,CANAL_ATD ,TP_AUTENTICACAO
UNION ALL
SELECT DISTINCT DAT_MOVIMENTO ,CANAL_ATD ,TP_AUTENTICACAO ,COUNT(DISTINCT(NUM_CONTRATO)) QT_CTR_UNICO  FROM TMP_SQDAA_COMP_ATM_DTH_P3C GROUP BY DAT_MOVIMENTO ,CANAL_ATD ,TP_AUTENTICACAO;
...
SELECT * FROM AGG_COMP_ATEND_DTH WHERE dat_movimento = TO_CHAR(TO_DATE(
				ADD_MONTHS(
					LAST_DAY(TO_DATE(TRUNC(&&DT_MOVIMENTO), 'YYYYMMDD')),-1)+1,'DD/MM/YYYY'));
